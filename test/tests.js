...
function defaultProps(pp = false) {
...
function print_index_signal() {
  const props = defaultProps();
  const s = props.calc_Coordinate_Transform(
    props.theta,
    props.phi,
    props.theta_s,
    props.phi_s,
...
function poling_period() {
  ...
  console.log(
    `Poling period: ${props.poling_sign >= 0 ? "" : "-"}${props.poling_period}`,
  );
...
function phasematch() {
  ...
  console.log("PMtz ", toPolar(amp_pm_tz));
  console.log("PM ", toPolar(amp));
...
function integrator_test() {
  ...
  let result = spdc.Nintegrate2D_3_8(
    (x, y) => {
...
function rates_test() {
  ...
  let dim = 15;
  let lamda_s = spdc.linspace(1546.01 * 1e-9, 1553.99 * 1e-9, dim);
  let lamda_i = spdc.linspace(1542.05 * 1e-9, 1550.0 * 1e-9, dim);
...
function heralding_apodization_test() {
  ...
  let dim = 30;
  let lamda_s = spdc.linspace(1490.86 * 1e-9, 1609.14 * 1e-9, dim);
  let lamda_i = spdc.linspace(1495.05 * 1e-9, 1614.03 * 1e-9, dim);
...
function test_for_krister() {
  ...
  let dim = 30;
  let lamda_s = spdc.linspace(1540.18 * 1e-9, 1559.82 * 1e-9, dim);
  let lamda_i = spdc.linspace(1540.3 * 1e-9, 1559.95 * 1e-9, dim);
...
function test_HOM() {
  ...
  props.auto_calc_collection_focus();
  props.update_all_angles();
  const ns = props.calc_Index_PMType(
    props.lambda_s,
    props.type,
    props.S_s,
    "signal",
  );
  const ni = props.calc_Index_PMType(
    props.lambda_i,
    props.type,
    props.S_i,
    "idler",
