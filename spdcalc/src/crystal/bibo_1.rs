...
///   ucum::ONE,
///   &[1.770147077637903, 1.7990347340642352, 1.93622182289392],
/// );
...
pub fn get_indices(wavelength : Wavelength, _temperature : ucum::Kelvin<f64>) -> Indices {
  ...
  let nx = ucum::ONE * (3.0740 + 0.0323 / (wl_sq - 0.0316) - 0.01337 * wl_sq).sqrt();

  let ny = ucum::ONE * (3.1685 + 0.0373 / (wl_sq - 0.0346) - 0.01750 * wl_sq).sqrt();

  let nz = ucum::ONE * (3.6545 + 0.0511 / (wl_sq - 0.0371) - 0.0226 * wl_sq).sqrt();
