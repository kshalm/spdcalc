...
fn get_rates_constant(spd : &SPD) -> f64 {
  ...
  let bw_pump = PI2c * 2. * fwhm_to_sigma(1. / (lamda_p - 0.5 * p_bw) - 1. / (lamda_p + 0.5 * p_bw));
...
pub fn calc_jsi_rates(spd : &SPD, cfg : &HistogramConfig) -> Vec<f64> {
  ...
  cfg
    ...
    .map(|(l_s, l_i)| {
      ...
      *(amplitude.norm_sqr() * factor * S / M / M )
    })
    .collect()
}

// calculate the coincidence rates
pub fn calc_jsi_singles_rates(spd : &SPD, cfg : &HistogramConfig) -> Vec<(f64, f64)> {
