        ...
        var jsahomUI = SkeletonUI.subclass({
            ...
            refreshLine: function( delT ){
                ...
                line.attr('x', function(d) {
                        // console.log("line x: ", self.plot1dEff.scales.x( d / delTConversion ) );
                        return self.plot1dEff.scales.x( d / delTConversion );
                    })
                    ;

                // console.log("Efficiency at this value: ", self.plot1dEff.scales.x.domain());
                var dom = self.plot1dEff.scales.x.domain();
                var val = ( delT / delTConversion );
                var indx = Math.floor( ((val - dom[0]) / (dom[1] - dom[0])) *(self.data1d.length-1));
                var eff = self.data1d[indx];
                console.log("Efficiency at this value: ", indx, eff, val-dom[0]);


                // Math.floor(((val-range[0])/(range[1]- range[0]))
                self.plot1dEff.setTitle("Collection Waist (um): " + eff['x'].toFixed(0) + ", Efficiency: " + eff['y'].toFixed(2));
                line.exit().remove();
            ...
            calc_HOM_JSA: function( props ){
                ...
                // self.plot.setXRange([ converter.to('nano', po.get('ls_start')), converter.to('nano', po.get('ls_stop')) ]);
                // self.plot.setYRange([ converter.to('nano', po.get('li_start')), converter.to('nano', po.get('li_stop')) ]);
                // self.plot1dEff.setTitle("Time delay = " + self.get('delT')/1e-15);
            },
