...
///
/// For convenience every function with the same signature as `value()`
/// qualifies as an objective function, e.g., minimizing a closure is perfectly
/// fine.
pub trait Function {
  /// Computes the objective function at a given `position` `x`, i.e., `f(x) =
  /// y`.
...
/// Defines an objective function `f` that is able to compute the first
/// derivative `f'(x)`.
...
/// ```
/// # use spdcalc::math::*;
/// let square = NumericalDifferentiation::new(Func(|x : &[f64]| x[0] * x[0]));
///
